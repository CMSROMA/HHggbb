...
int main( int argc, char* argv[] ) {
  ...
  if( argc!=4 && argc!=5 ) {
    std::cout << "USAGE: ./forMjjStudies_radion [redntpVersion] [dataset] [selectionType] [bTaggerType=JP or CSV]" <<std::endl;
    return 13;
  }
...
void finalize_oneDataset( const std::string& redntpProdVersion, const std::string& dataset, const std::string& selectionType, const std::string& bTaggerType, std::vector<std::string> *datasets ) {
  ...
  bestJets* rf = new bestJets( dataset, selectionType, bTaggerType );
